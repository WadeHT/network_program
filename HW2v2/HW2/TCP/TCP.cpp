#include "TCP.h"
int Send(string str,int fd)
{
	return send(fd,(char*)str.c_str(),(int)str.length(),0);
}
int Send(fstream &fs,int csock,int bufSize)
{
	int result = 0,each;
	char buf[bufSize];
	while(!fs.eof()  && fs.is_open())
	{
		...
		fs.read(buf,bufSize);
		if((each = send(csock,buf,bufSize,0)) < 0)return each;
		result += each;
	}
	return result;
}
string Recv(int csock,int bufSize)
{
	char temp[bufSize] = {};
	if( recv(csock,temp,bufSize,0) < 0)return "";
	return (string)temp;
}

/*================================sTCP=============================*/
sTCP::sTCP(int family,int type,int protocol):sockfd(socket(family,type,protocol)),clen(sizeof(caddr)),on(1),buf(new char[buffSize0]),bufSize(buffSize0)
{
...
int sTCP::Send(string str,int fd)
{
	if(fd == -1)fd = csock;
	return send(fd,(char*)str.c_str(),(int)str.length(),0);
}
...
sTCP& sTCP::operator=(const sTCP& right)
{
	sockfd = right.sockfd;
	csock = right.csock;
	buf = new char[right.bufSize];
	myaddr = right.myaddr;
	caddr = right.caddr;
	clen = right.clen;
	mesg = right.mesg;
...
int cTCP::Send(string str,int fd)
{
	if(fd == -1)fd = sockfd;
	return send(fd,(char*)str.c_str(),(int)str.length(),0);
}
